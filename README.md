## go-cpp-sample [![Go Reference][1]][2]

`go-cpp-sample` contains a simple example calling C++ from Go.
...
Please run it using linux or WSL on Windows environments.
...
## add the desired path, export it and try the application.
LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/path-to/go-cpp-sample/
